...
def solve_model(model_extension, time_array, parameters, initial_condition, treatment_days, dose, type_sol='data'):
    ...
    if type_sol == 'smooth':
        ...
        sol = odeint(model_func, t=time_array, y0=[initial_condition], args=(*parameters, tau_imuno, dose_schedule), mxstep=2000)
        return np.column_stack((time_array, sol))
    else:
        return odeint(model_func, t=time_array, y0=[initial_condition], args=(*parameters, tau_imuno, dose_schedule), mxstep=2000)
...
def log_likelihood(theta, model_extension, full_data, group, mouse_index):
    ...
    treatment_params = tuple(theta[:-2])
...
def plot_maxll_solution(files_location, full_data, nCols=3, show=True, save=True, fontsize='14', figure_name='maxll_figure'):
    ...
    for idx, chain in enumerate(all_chains):
        ...
        group = f"{chain_split[3]}_{chain_split[4]}"

        mouse_name = f"{chain_split[5]}_{chain_split[6]}"
        ...
        ax.text(0.05, .9, '{:}) {:}'.format(string.ascii_uppercase[idx], mouse_name),
        ...
        print(f"Parameters loaded for the {model_extension} model and mouse {mouse_name}: {theta}")
...
def define_bounds_labels(model_extension, bool_multiple_mice=False, group_size=1):
    labels = []
    l_bound = []
    u_bound = []
    ic_max = max_ic*2
...
if __name__ == "__main__":
    ...
    if run_calibration:
        for model_extension in [
            ...
            l_bound, u_bound, labels = define_bounds_labels(model_extension, bool_multiple_mice=False, group_size=1)
            ...
            for group in ['io_sensitive']:
                print(f'Processing group: {group}, using model {model_extension}')
                for mouse_index, mouse in enumerate(full_data[group]):
                    mouse_name = mouse['name']
 
                    filename = f"./reinitiate_files/reinitiate_{group}_{mouse_name}{model_extension}.h5"

                    backend = emcee.backends.HDFBackend(filename)
                    if not os.path.exists(filename):
                        print(f"HDF5 file not found for {mouse_name}. Creating new backend...")
                        ...
                        print(f"HDF5 file found for {mouse_name}: {filename}")
                        print(f"  Current iterations in backend: {backend.iteration}")
                        pos = None 

                    additional_chain_size = 10 

                    print(f'Calibrating {group}, mouse {mouse_name}, model {model_extension}')
                    ...
                    np.savetxt(f'./Output_Calibration/chain_{group}_{mouse_name}{model_extension}.gz', save_chain)
                    np.savez(f'./Output_Calibration/ll_pars_{group}_{mouse_name}{model_extension}.npz', max_ll=max(flat_ll), pars=best_pars)

                    # corner_chain = sampler.get_chain(discard=additional_chain_size // 2, flat=True)
                    # fig = corner.corner(corner_chain, labels=labels)
                    # plt.savefig('./Output_Calibration/corner_' + group + '_' + mouse_name + model_extension + '.png', format='png', dpi=300, bbox_inches='tight', pad_inches=0.02)
                    # plt.close()
                    ...
                    plt.savefig('./Output_Calibration/chain_' + group + '_' + mouse_name + model_extension + '.png', format='png', dpi=300, bbox_inches='tight', pad_inches=0.02)
                
                    plt.close()
                
                plot_maxll_solution('./Output_Calibration/ll_pars_' + group + '_*' + model_extension + '.npz', full_data, show = False, nCols=4, figure_name='./Output_Calibration/max_ll_' + group + model_extension)