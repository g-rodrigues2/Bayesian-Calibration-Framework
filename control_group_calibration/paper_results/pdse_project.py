...
def color_plot_scatter_suv(suv_measured, suv_calibrated, save, show, figure_name, formatter):
    """
    Gera um scatter-plot com linha de identidade para comparar
    SUV medido (eixo X) vs. SUV calibrado (eixo Y).
    
    Parâmetros:
    -----------
    suv_measured : array-like
        Valores de SUV medidos (ex.: obtidos de imagem).
    suv_calibrated : array-like
        Valores de SUV calibrados (resultado da estimação de parâmetros).
    save : bool
        Se True, salva a figura em disco.
    show : bool
        Se True, mostra a figura na tela.
    figure_name : str
        Nome base do arquivo (caso seja salvo).
    formatter : matplotlib.ticker.Formatter
        Formatter para padronizar os eixos (notação científica, etc.).
    """
    # Cria a figura
    fig, ax = plt.subplots(figsize=(5, 5))

    # Faz o scatter de todos os pontos
    ax.plot(suv_measured, suv_calibrated, 'o', color='blue', alpha=0.8)

    # Eixos e rótulos
    ax.set_xlabel('Measured SUV')
    ax.set_ylabel('Calibrated $\\gamma$')

    # Linha de identidade (y=x), para referência
    line = mlines.Line2D([0, 1], [0, 1], color='black', linestyle='dashed')
    line.set_transform(ax.transAxes)
    ax.add_line(line)

    # Define limites de acordo com o máximo encontrado
    max_value = max(suv_measured.max(), 2.0)
    max_value = math.ceil(max_value / 0.5) * 0.5  # Ex.: arredonda para 0.5 ou 1.0

    ax.set_xlim((0, 2.0))
    ax.set_ylim((0, 6.5))

    # Aplicar formatação de eixos
    ax.xaxis.set_major_formatter(formatter)
    ax.yaxis.set_major_formatter(formatter)

    # Calcula métricas agregadas (para todos os pontos)
    ccc_val = ccc(suv_measured, suv_calibrated)
    pcc_val = pcc(suv_measured, suv_calibrated)
    mape_val = mape(suv_measured, suv_calibrated)

    # Escreve métricas no canto do plot
    ax.text(
        0.05, 0.90,
        f'CCC = {ccc_val:.2f}\nPCC = {pcc_val:.2f}\nMAPE = {mape_val:.2f}%',
    ...
    # Salvar ou mostrar
    if save:
        plt.savefig(figure_name + '_SUV_sp_pol1.pdf', bbox_inches='tight', pad_inches=0.02)
    if show:
        plt.show()
    else:
        plt.close()
...
#--------------- Função para comparar diferentes modelos no mesmo gráfico --------------------------

def plot_maxll_solution_compare_models(files_location, full_data, nCols=3, show=True, save=True, fontsize='14', figure_name='maxll_figure'):
    ...
    # Variáveis para armazenar o arquivo de maior likelihood por grupo e por modelo
    best_chain_per_model_group = {"_LQtempdelay": {}, "_LQcumulativedelay": {}}
    max_ll_per_model_group = {"_LQtempdelay": {}, "_LQcumulativedelay": {}}

    # Iterar sobre todos os arquivos .npz e encontrar o melhor ajuste (maior likelihood) para cada modelo por grupo
    for chain in all_chains:
        npzfile = np.load(chain)
        max_ll = npzfile['max_ll']

        # Extração do grupo e do modelo do nome do arquivo
        chain_split = chain.split('_')
        group = f"{chain_split[4]}_{chain_split[5]}"  # Exemplo: radiation_sensitive ou radiation_resistant
        model_extension = f"_{chain_split[-1].split('.')[0]}"  # Exemplo: _LQtempdelay ou _LQcumulativedelay

        # Verificar se é o primeiro arquivo do grupo/modelo ou se o max_ll é maior que o anterior
        if group not in max_ll_per_model_group[model_extension] or max_ll > max_ll_per_model_group[model_extension][group]:
            max_ll_per_model_group[model_extension][group] = max_ll
            best_chain_per_model_group[model_extension][group] = chain

    max_times = find_max_time_per_group(full_data)

    # Parâmetros fixos para cada grupo
    growth_rates = {
        'radiation_sensitive': 8.89429860e-02,  # Crescimento para radiation_sensitive
        'radiation_resistant': 1.15631515e-01  # Crescimento para radiation_resistant
    }
    carrying_capacity = 3.69671552e+03  # Capacidade de suporte

    # Iterar separadamente sobre cada grupo
    for group in ['radiation_sensitive', 'radiation_resistant']:
        print(f"Processando: Grupo = {group}")

        # Configurar os subplots para cada grupo
        ...
        full_exp = []
        full_model = []
        # Define limite do eixo Y com base no grupo
        y_limit = 2000 if group == 'radiation_sensitive' else 3000

        # Loop para os dois modelos (_LQtempdelay e _LQcumulativedelay)
        for model_extension in ["_LQtempdelay", "_LQcumulativedelay"]:
            # Carregar o arquivo .npz correspondente ao melhor ajuste do grupo e modelo
            best_chain = best_chain_per_model_group[model_extension].get(group)
            if not best_chain:
                continue  # Pular se não houver arquivo correspondente para esse grupo e modelo

            npzfile = np.load(best_chain)
            theta = npzfile['pars']
            print(f"Melhor ajuste para {model_extension} no grupo {group}: {theta}")

            ndim = len(full_data[group])
            max_time_group = max_times[group]

            # Parâmetro de taxa de crescimento para o grupo específico
            growth_rate = growth_rates[group]
            ...
            for idx, mouse_data in enumerate(full_data[group]):
                ...
                # Plotar os dados reais (exp) apenas uma vez por rato
                if model_extension == "_LQtempdelay":  # Condição para evitar repetição
                ...
                # Plotar as linhas verticais para os dias de tratamento
                ...
                linestyle = 'dashed' if model_extension == "_LQtempdelay" else 'dotted'
                label = "Time repair with delay" if model_extension == "_LQtempdelay" else "Cumulative dose with delay"

                # Extrair os parâmetros de tratamento estimados
                treatment_params = tuple(theta[:-(ndim + 1)])
                initial_condition = theta[-(ndim + 1) + idx]

                # Print para verificação de parâmetros
                print(f"\nCamundongo {mouse_name} - Grupo: {group}")
                print(f"Modelo: {model_extension}")
                print(f"Parâmetros de tratamento: {treatment_params}")
                print(f"Condição inicial: {initial_condition}")

                # Chamar solve_model com os parâmetros fixos e estimados
                ...
                # Plotar a solução do modelo ajustado com o estilo específico do modelo
                ax.plot(matched_solution_times, matched_solution_volumes, linestyle=linestyle, linewidth=2, label=label)
                
                # Adicionar a legenda para cada subplot
                if idx == 0:
                    axes[0].legend()

            #Esconder subplots não utilizados
            for j in range(nScenarios, len(axes)):
                axes[j].axis('off')
        # Finalizar o gráfico para o grupo atual
        group_figure_name = f"comparison_{group}.pdf"  # Nome específico para o grupo
        ...
        if save:
            plt.savefig(group_figure_name, format='pdf')
            print(f"Gráfico salvo como {group_figure_name}")
