...
def log_likelihood(theta, model_extension, full_data, group, mouse_index):
    ...
    carrying_capacity = 3.69671552e+03  # Shared carrying capacity
    ...
    for idx, chain in enumerate(all_chains):
        ...
        if group in full_data:
            ...
            if mouse_data is None:
                print(f"Mouse {mouse_name} not found in the group {group}")
                continue
        else:
        ...
        print(f"Parameters loaded into the model {model_extension} and mouse {mouse_name}: {theta}")
    ...
    if run_calibration:
        for model_extension in [
            ...
            for group in ['radiation_sensitive', 'radiation_resistant']:
                print(f'Processing group: {group}, using model {model_extension.split("_")[-1]}')
                for mouse_index, mouse in enumerate(full_data[group]):
                    ...
                    total_size = ((chain_size-chain_size//4)*ndim*2)
                    goal_size = 10000
                    scale_corner = max(1, total_size // goal_size)
                    save_chain = sampler.get_chain(discard=chain_size // 4, flat=True, thin = scale_corner)
